...
import core.Mono.just
import core.{Flux, JavaInterop, Mono, Scannable}
...
class SMonoTest extends AnyFreeSpec with Matchers with TestSupport with IdiomaticMockito with ArgumentMatchersSugar {
  ...
  "Mono" - {
    ".create should create a Mono" in {
      StepVerifier.create(Mono.create[Long](monoSink => monoSink.success(randomValue)))
    ...
    ".defer should create a Mono with deferred Mono" in {
      StepVerifier.create(Mono.defer(() => just(randomValue)))
        .expectNext(randomValue)
        .expectComplete()
        .verify()
    }
    ...
    ".from" - {
      "a publisher should ensure that the publisher will emit 0 or 1 item." in {
        StepVerifier.create(Mono.from(Flux.just(1, 2, 3, 4, 5)))
          .expectNext(1)
          .expectComplete()
          .verify()
      }
      ...
      "a callable should ensure that Mono will return a value from the Callable" in {
        StepVerifier.create(Mono.fromCallable(() => randomValue))
          .expectNext(randomValue)
          .expectComplete()
          .verify()
      }
...
//      }
    ...
    ".ignoreElements should ignore all elements from a publisher and just react on completion signal" in {
      StepVerifier.create(Mono.ignoreElements(just(randomValue)))
        .expectComplete()
        .verify()
    }
    ...
    ".justOrEmpty" - {
      "with Option should" - {
        "emit the specified item if the option is not empty" in {
          StepVerifier.create(Mono.justOrEmpty(Option(randomValue)))
            .expectNext(randomValue)
            .verifyComplete()
        }
        "just react on completion signal if the option is empty" in {
          StepVerifier.create(Mono.justOrEmpty(Option.empty))
            .expectComplete()
            .verify()
        }
      }
...
//    }
    ...
    ".sequenceEqual should" - {
      "emit Boolean.TRUE when both publisher emit the same value" in {
        StepVerifier.create(Mono.sequenceEqual(just(1), just(1)))
          .expectNext(true)
          .verifyComplete()
      }
    ...
    ".raiseError should create Mono that emit error" in {
      StepVerifier.create(Mono.error(new RuntimeException("runtime error")))
    ...
    ".when" - {
      ...
      "with varargs of publisher should return when all of the resources has fulfilled" in {
        ...
        StepVerifier.create(Mono.when(sources.toArray: _*))
          .expectComplete()
        completed should contain key "first"
        completed should contain key "second"
      }
    ...
    ".zipDelayError" - {
      ...
      "with p1, p2, p3, p4 and p5 should merge when all Monos are fulfilled" in {
        StepVerifier.create(Mono.zipDelayError(just[Int](1), just(2), just(3), just(4), just(5)))
      ...
      "with iterable" - {
        ...
        "of combinator function and monos should emit the value after combined by combinator function" in {
          StepVerifier.create(Mono.zipDelayError((values: Array[AnyRef]) => s"${values(0).toString}-${values(1).toString}", just(1), just("one")))
            .expectNext("1-one")
            .verifyComplete()
        }
      }
    ...
    ".as should transform the Mono to whatever the transformer function is provided" in {
      ...
      StepVerifier.create(mono.as(m => Flux.from(m)))
        .expectNext(randomValue)
        .verifyComplete()
    }
...
//    }
//
//    ".asScala should transform Mono to Mono" in {
...
////      new ReactiveMono[Int](jMono).cancelOn(Schedulers.immediate())
//      JavaInterop.wrapMono(jMono).cancelOn(Schedulers.immediate())
//      jMono.cancelOn(any[Scheduler]) was called
//    }

    // depreciated
//    ".compose should defer creating the target mono type" in {
//      StepVerifier.create(just(1).compose[String](m => Flux.fromPublisher(m.map(_.toString))))
//        .expectNext("1")
//        .verifyComplete()
//    }
    ".transformDeferred should defer creating the target mono type" in {
      StepVerifier.create(just(1).transformDeferred[String](m => Flux.from(m.map(_.toString))))
        .expectNext("1")
        .verifyComplete()
    }
    ...
    ".defaultIfEmpty should use the provided default value if the mono is empty" in {
      StepVerifier.create(Mono.empty[Int].defaultIfEmpty(-1))
        .expectNext(-1)
        .verifyComplete()
    }
    ...
    ".delayUntil should delay until the other provider terminate" in {
      StepVerifier.withVirtualTime(() => just(randomValue).delayUntil(_ => Flux.just(1, 2).delayElements(2 seconds)))
        .thenAwait(4 seconds)
        .expectNext(randomValue)
        .verifyComplete()
    }
    ...
    ".flatMap should flatmap the provided mono" in {
      StepVerifier.create(just(randomValue).flatMap(l => just(l.toString)))
    ...
    ".flatMapMany" - {
      "with a single mapper should flatmap the value mapped by the provided mapper" in {
        StepVerifier.create(just(1).flatMapMany(i => Flux.just(i, i * 2)))
          .expectNext(1, 2)
          .verifyComplete()
      }
    ...
    ".hasElement should convert to another Mono that emit" - {
      ...
      "false if it is empty" in {
        StepVerifier.create(Mono.empty.hasElement)
          .expectNext(false)
          .verifyComplete()
      }
    }
    ...
    ".or should return Mono that emit the value between the two Monos that is emited first" in {
      StepVerifier.create(Mono.delay(5 seconds).or(just(2)))
        .expectNext(2)
        .verifyComplete()
    }
    ...
    ".repeatWhen should emit the value of this mono accompanied by the publisher" in {
      StepVerifier.create(just(randomValue).repeatWhen((_: Flux[Long]) => Flux.just[Long](10, 20)))
        .expectNext(randomValue, randomValue, randomValue)
        .verifyComplete()
    }
...
//    }
    ...
    ".takeUntilOther should complete if the companion publisher emit any signal first" in {
      StepVerifier.withVirtualTime(() => Mono.delay(10 seconds).takeUntilOther(just("a")))
        .verifyComplete()
    }
    ...
    ".thenMany should ignore the element from this mono and transform the completion signal into a Flux that will emit " +
      ...
      "directly" in {
        StepVerifier.create(just(randomValue).thenMany(Flux.just(1, 2, 3)))
          .expectNext(1, 2, 3)
          .verifyComplete()
      }
    }
