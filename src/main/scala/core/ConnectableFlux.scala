...
class ConnectableFlux[T](private val connectableDelegate: JConnectableFlux[T]) extends Flux[T] {
  ...
  final def connect(cancelSupport: Disposable => Unit): Unit = connectableDelegate.connect(cancelSupport)
  ...
  final def refCount(minSubscribers: Int, gracePeriod: FiniteDuration): Flux[T] = Flux.from(connectableDelegate.refCount(minSubscribers, gracePeriod))
  final def refCount(minSubscribers: Int, gracePeriod: FiniteDuration, scheduler: Scheduler): Flux[T] = Flux.from(connectableDelegate.refCount(minSubscribers, gracePeriod, scheduler))
