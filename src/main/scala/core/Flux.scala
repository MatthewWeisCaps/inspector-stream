...
trait Flux[T] extends Publisher[T] with ImplicitJavaInterop {
  ...
  def failAndSupply(messageSupplier: () => String): Flux[T] = wrapFlux[T](delegate.doOnNext(_ => Assertions.fail(messageSupplier)))
  ...
  def assertAlwaysWithMsg(predicate: T => Boolean, message: String): Flux[T] = wrapFlux[T](delegate.doOnNext(it => Assertions.assertTrue(predicate(it), message)))
  def assertAlwaysAndSupply(predicate: T => Boolean, messageSupplier: () => String): Flux[T] = wrapFlux[T](delegate.doOnNext(it => Assertions.assertTrue(predicate(it), messageSupplier())))
  def assertAlwaysWithFn(predicate: T => Boolean, messageFn: T => String): Flux[T] = wrapFlux[T](delegate.doOnNext(it => Assertions.assertTrue(predicate(it), messageFn(it))))

  def assertNever(predicate: T => Boolean): Flux[T] = wrapFlux[T](delegate.doOnNext(it => Assertions.assertFalse(predicate(it))))
  def assertNeverWithMsg(predicate: T => Boolean, message: String): Flux[T] = wrapFlux[T](delegate.doOnNext(it => Assertions.assertFalse(predicate(it), message)))
  def assertNeverAndSupply(predicate: T => Boolean, messageSupplier: () => String): Flux[T] = wrapFlux[T](delegate.doOnNext(it => Assertions.assertFalse(predicate(it), messageSupplier())))
  def assertNeverWithFn(predicate: T => Boolean, messageFn: T => String): Flux[T] = wrapFlux[T](delegate.doOnNext(it => Assertions.assertFalse(predicate(it), messageFn(it))))
